...
from quotes_js_scraper.spiders.imagekit import imagekit_image_upload

class QuotesScrollSpider(scrapy.Spider):

    # logger
    logger = logging.getLogger('imgkit')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler('imgkit.log')
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)

    IMAGEKIT = imagekit_image_upload()
    DB = CRUD_DB()
    name = 'imgkit'
    platform = 'amazon'
    ...
    async def parse_product_details(self, response):
        ...
        if(data_in_db):
            ...
            if(image_url == self.DB.get_image_url_from_db(product_name, self.platform)):
                ...
                if(image_id != None):
                    image_id, image_name, backend_image_url, thumbnail_image_url = self.IMAGEKIT.upload_image_and_get_url(image_url, self.platform)
                    if(image_id!=None):
